...
class SE23_se3_cal_equi_EqF:
    ...
    def update(self, vel : np.ndarray, omega_noise : float, acc_noise : float, tau_noise : float, virtual_noise : float, y : np.ndarray, meas_noise : float, dt : float):
        ...
        if not self.propagation_only:
            if not np.isnan(y[0,0:1]):
                # tmp = self.outputMatrixCStar(y)
                Ct = np.hstack((0.5*(SO3.wedge(y + (self.X_hat.B.w().as_vector() - SO3.vee(self.X_hat.gamma)))),
                                   np.zeros((3, 3)),
                                   -np.eye(3),
                                   np.zeros((3, 6)),
                                   np.eye(3)))
                # Ct = np.hstack((SO3.wedge(y),
                #                 np.zeros((3, 3)),
                #                 -np.eye(3),
                #                 np.zeros((3, 6)),
                #                 np.eye(3)))
                delta = outputAction(self.X_hat.inv(), np.zeros((3, 1))) - y #rho_hat{X^{-1}}(0)-y
    ...
    def outputMatrixCStar(self, y : np.ndarray) -> np.ndarray:
        ...
        CStar_1 = numericalDifferential(opf_1, np.zeros((18,1)))
        CStar_2 = numericalDifferential(opf_2, np.zeros((18, 1)))
