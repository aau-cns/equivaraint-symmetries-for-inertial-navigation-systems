...
from Symmetries.Calibration.SE3_se3_R3.Symmetry import *
...
def continuous_lift(xi : State, U : InputSpace) -> np.ndarray:
    ...
    L[12:15, 0:1] = xi.T.R().as_matrix() @ ((U.w - SO3.wedge(xi.b[0:3, 0:1])) @ (xi.t + U.k) + U.mu) + xi.T.x().as_vector()
    L[15:18, 0:1] = -(U.eta + ((U.w - SO3.wedge(xi.b[0:3, 0:1])) @ xi.t))

    return L


class SE3_se3_R3_cal_EqF:
    def __init__(self, initial_nav_noise=1.0, initial_bias_noise=0.01, propagationonly=False, curvature_correction=False):
        self.X_hat = SymGroup()

        sigma_vec = np.concatenate((np.ones((1, 6)) * initial_nav_noise ** 2, np.ones((1, 6)) * initial_bias_noise ** 2, np.ones((1, 3)) * initial_nav_noise ** 2, np.ones((1, 3)) * (initial_nav_noise / 5) ** 2), axis=1)
    ...
    def getEstimate(self):
        ...
        p = xi_hat.r - (xi_hat.T.R().as_matrix() @ xi_hat.t)
    ...
    def update(self, vel : np.ndarray, omega_noise : float, acc_noise : float, tau_noise : float, virtual_noise : float, y : np.ndarray, meas_noise : float, dt : float):
        ...
        # Settings
        noise_vec = np.concatenate((np.ones((1, 3)) * omega_noise ** 2, np.ones((1, 3)) * acc_noise ** 2, np.ones((1, 6)) * (tau_noise) ** 2, np.ones((1, 9)) * virtual_noise ** 2), axis=1)
        ...
        if not self.propagation_only:
            if not np.isnan(y[0,0:1]):
                Ct = np.hstack((np.zeros((3, 12)), np.eye(3), np.zeros((3, 3))))
    ...
    def stateMatrixA_CT(self, u : InputSpace) -> np.ndarray:
        ...
        Gamma = np.hstack((-SO3.wedge((u_0.w @ u_0.k) + u_0.mu), np.eye(3)))
        Xi = np.hstack((-SO3.wedge(u_0.k), np.zeros((3, 3))))
    ...
    def inputMatrixBt_CT(self, u : InputSpace) -> np.ndarray:     
        ...
        u0 = velocityAction(self.X_hat.inv(),u)
        Bt = np.zeros((18, 21))
        Bt[0:6, 0:6] = self.X_hat.B.Adjoint()
        Bt[6:12, 6:12] = -self.X_hat.B.Adjoint()
        Bt[12:15, 0:3] = -SO3.wedge(u0.k) @ self.X_hat.B.R().as_matrix()
        Bt[12:15, 12:15] = self.X_hat.B.R().as_matrix()
        Bt[12:15, 15:18] = u0.w @ self.X_hat.B.R().as_matrix()
        Bt[15:18, 18:21] = -self.X_hat.B.R().as_matrix()
        return Bt
...
class TestGroup(unittest.TestCase):
    ...
    def assertStateEqual(self, S1: State, S2: State):
        ...
        self.assertMatricesEqual(S1.r, S2.r)
    ...
    def continuousTimeDinamics(self, xi : State, U : InputSpace) -> np.ndarray:
        dot_T = SE3.vee(xi.T.as_matrix() @ (U.as_wa_mat() - SE3.wedge(xi.b)) + G)
        dot_r = xi.T.R().as_matrix() @ ((U.w - SO3.wedge(xi.b[0:3, 0:1])) @ (xi.t + U.k) + U.mu) + xi.T.x().as_vector()
        dot_b = U.tau
        dot_t = U.eta
        return np.vstack((dot_T, dot_b, dot_r, dot_t))
